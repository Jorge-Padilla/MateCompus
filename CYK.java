...
public class CYK {
    ...
    public FNCh gramatica;
    public char[] cadena;
    public boolean valid;

    public CYK(File gramatica, String cadena) throws IOException {
        this.gramatica = new FNCh(gramatica);
    ...
    public boolean cyk(){
        ...
        for(int i = 0; i < cadena.length; i++){
            for(Simbolo a : gramatica.gramatica){
        ...
        for(Simbolo a : matrix[0][cadena.length-1]){
            if(a.equals(this.gramatica.gramatica.get(0))){
                System.out.println("La cadena es valida, este es su arbol de derivación:");
                System.out.println(trees[0][cadena.length-1].get(0) + "\n");
                return true;
            }
        }
        System.out.println("La cadena NO es valida en la Gramática");
        return false;
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        if(returnVal == JFileChooser.APPROVE_OPTION) {
            System.out.println("El archivo que desea abrir: " +
                    chooser.getSelectedFile().getName());
        }
        String word = JOptionPane.showInputDialog("Inserte la cadena");
        CYK c0 = new CYK(new File(chooser.getSelectedFile().getName()), word);

        //CYK c = new CYK(new File("Gramatica"), "aabbab");
        CYK c2 = new CYK(new File("Gramatica2"), "cabaac");
